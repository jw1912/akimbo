...
fn parse_go(pos: &mut Position, commands: Vec<&str>, ctx: &mut SearchContext) {
    ...
    if times[pos.side_to_move] != 0 {
        ctx.alloc_time = times[pos.side_to_move] as u128 / (if let Some(mtg) = moves_to_go {mtg as u128} else {2 * (pos.phase as u128 + 1)}) - 10;
    }
    go(pos, depth, ctx);
}
...
fn parse_fen(s: &str) -> Position {
    ...
    let mut pos: Position = Position { pieces: [0; 6], sides: [0; 2], squares: [EMPTY as u8; 64], side_to_move: 0, state: GameState::default(), nulls: 0, stack: Vec::new(), phase: 0 };
