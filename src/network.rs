...
static NNUE: Network = unsafe { std::mem::transmute(*include_bytes!("../resources/morelayers-8-crelu-16-crelu-epoch17.bin")) };
...
fn _quantise() {
    ...
    static RAW_NET: [f32; SIZE] = unsafe {
        std::mem::transmute(*include_bytes!("../../bullet/checkpoints/morelayers-8-crelu-16-crelu-epoch17/params.bin"))
    };

    let mut file = File::create("resources/morelayers-8-crelu-16-crelu-epoch17.bin").unwrap();
